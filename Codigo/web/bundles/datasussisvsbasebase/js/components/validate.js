...
var Validation = {
    ...
    errorElement: 'span',
    initializer: false,
    errorLabelContainer: $('div.alert-error'),
    messages: {
        ano: 'O ano informado não é válido.',
        required: 'O campo __campo__ é de preenchimento obrigatório.',
        remote: "",
    ...
    unhighlight: function (element) {
        ...
        if(true === this.check(element)){
            if(Message.message[$(element).attr('id')]){
                Message.remove($(element).attr('id'));
            }
        }
    },
    ...
    init: function () {
        ...
        $.validator.setDefaults({
            errorPlacement: function (error, element) {
                ...
                var nome = $.trim($('label[for=' + $(element).attr('id') + ']:first').text().replace(':', '').replace('*', ''));

                Message.addMessage(message.replace('__campo__', nome), 'error', $(element).attr('id'));
            }
        });
    ...
    addMessage: function (message) {
        ...
        setTimeout(function () {
            $('div.message div.alert-error').html('').hide();
        }, 6000);
    }
}
