...
use Datasus\Core\BaseBundle\ServiceLayer\MessageService;
use Datasus\Core\BaseBundle\Controller\ControllerAbstract;
use Datasus\Core\BaseBundle\Entity\AbstractBase;
use Symfony\Component\HttpFoundation\Response;

class ControllerCrudAbstract extends ControllerAbstract
{
    /**
    ...
    {
        if ($this->get('session')->has('DatasusSaveResult') && $this->get('session')->get('DatasusSaveResult') instanceof AbstractBase) {
            $entityData = $this->get('session')->get('DatasusSaveResult')->toArray();
        } else {
            $entityData = $this->getService()->getRootEntityData();
        }

        if ($this->get('session')->has('DatasusSaveResult')) {
            $this->get('session')->remove('DatasusSaveResult');
        }
    ...
    public function saveAction()
    {
        try {
            ...
            $this->addMessage('Dados gravados com sucesso.', 'success');
            $this->get('session')->set('DatasusSaveResult', true);
            return $this->redirectByRouteName($this->saveSuccessRoute);
        } catch (\Exception $e) {
        	$this->get('session')->set('DatasusSaveResult', $this->getService()->getRootEntity());
